...
const RubricsPage = () => {
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            ...
            // const token = localStorage.getItem('token'); // Removed manual token management
            // if (!token) return; // Removed manual token management

            const res = await api.get(`/rubrics`); // Changed to api.get and removed headers
            setRubrics(res.data);
        } catch (error) {
            console.error(error);
            toast.error('Error al cargar rúbricas');
