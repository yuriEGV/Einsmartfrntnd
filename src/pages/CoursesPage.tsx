            ...
            {loading ? (
                ...
                <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                    {filteredCourses.map((course, idx) => (
                        <div
                            key={course._id}
                            className="bg-white p-6 rounded-[2.5rem] shadow-2xl shadow-blue-900/5 border border-slate-50 hover:translate-y-[-5px] transition-all group relative overflow-hidden flex flex-col"
                            ...
                            <div className="relative z-10 flex flex-col h-full">
                                <div className="flex items-start justify-between mb-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-16 h-16 rounded-3xl bg-blue-50 text-[#11355a] flex items-center justify-center font-black group-hover:bg-[#11355a] group-hover:text-white transition-all shadow-sm border-2 border-white">
                                            <BookOpen size={28} />
                                        </div>
                                        <div>
                                            ...
                                            <div className="inline-block px-2 py-0.5 bg-slate-100 rounded text-[9px] font-mono font-black text-slate-400 uppercase tracking-tighter mt-1">
                                                ID: {course.code}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <p className="text-slate-500 text-sm font-bold leading-relaxed mb-6 line-clamp-2 min-h-[3rem]">
                                    {course.description || 'Sin descripción detallada para este nivel.'}
                                </p>

                                <div className="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between">
                                    <div className="flex items-center gap-3">
                                        <div className="w-10 h-10 rounded-full bg-blue-100 border-2 border-white overflow-hidden flex items-center justify-center text-[#11355a] font-black text-xs shadow-sm">
                                            {course.teacherId?.name ? course.teacherId.name.charAt(0) : '?'}
                                        </div>
                                        <div className="min-w-0">
                                            <div className="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none mb-1">Docente Guía</div>
                                            <div className="text-xs font-black text-slate-600 truncate max-w-[120px]">
                                                {course.teacherId?.name || 'VACANTE'}
                                            </div>
                                        </div>
                                    </div>
                                    <div className="px-3 py-1 bg-slate-50 rounded-lg border border-slate-100 font-black text-[9px] text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
