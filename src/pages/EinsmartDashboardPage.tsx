...
export default function EinsmartDashboardPage({ stats }: { stats: any }) {
    ...
    return (
        <div className="space-y-4 md:space-y-6 p-4 md:p-6 animate-in fade-in duration-700 bg-slate-50 min-h-full">
            
            <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4 bg-white md:bg-transparent p-4 md:p-0 rounded-2xl shadow-sm md:shadow-none border md:border-none">
                <div className="space-y-1">
                    <h1 className="text-xl md:text-3xl font-black text-slate-800 tracking-tight leading-tight uppercase">
                        <span className="block md:inline text-emerald-600 opacity-90">Einsmart</span> Overlord
                    </h1>
                    <p className="text-[9px] md:text-xs text-slate-400 font-black uppercase tracking-[0.2em] flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                <div className="bg-white p-5 md:p-6 rounded-3xl shadow-xl shadow-slate-200/40 border border-slate-100 hover:-translate-y-1 transition-all group">
                    <div className="flex items-center justify-between mb-4">
                        <div className="p-2.5 bg-emerald-50 rounded-xl text-emerald-600"><School size={20} /></div>
                        <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest bg-slate-50 px-3 py-1 rounded-full">Clientes</span>
                    </div>
                    <p className="text-4xl font-black text-slate-800 tracking-tighter">{stats?.tenantCount || 0}</p>
                    <p className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-tight">Colegios en el Nodo</p>
                </div>

                <div className="bg-white p-5 md:p-6 rounded-3xl shadow-xl shadow-slate-200/40 border border-slate-100 hover:-translate-y-1 transition-all group">
                    <div className="flex items-center justify-between mb-4">
                        <div className="p-2.5 bg-blue-50 rounded-xl text-blue-600"><Users size={20} /></div>
                        <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest bg-slate-50 px-3 py-1 rounded-full">Matrícula</span>
                    </div>
                    <p className="text-4xl font-black text-slate-800 tracking-tighter">{stats?.studentCount?.toLocaleString() || 0}</p>
                    <p className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-tight">Estudiantes Sincronizados</p>
                </div>

                <div className="bg-white p-5 md:p-6 rounded-3xl shadow-xl shadow-slate-200/40 border border-slate-100 hover:-translate-y-1 transition-all group">
                    <div className="flex items-center justify-between mb-4">
                        <div className="p-2.5 bg-purple-50 rounded-xl text-purple-600"><Box size={20} /></div>
                        <span className="text-[9px] font-black text-slate-300 uppercase tracking-widest bg-slate-50 px-3 py-1 rounded-full">Cursos</span>
                    </div>
                    <p className="text-4xl font-black text-slate-800 tracking-tighter">{stats?.courseCount || 0}</p>
                    <p className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-tight">Cursos Aislados</p>
                </div>

                <div className="bg-slate-900 p-5 md:p-6 rounded-3xl shadow-2xl shadow-slate-900/50 hover:shadow-emerald-900/40 border border-slate-800 hover:-translate-y-1 transition-all group flex flex-col justify-between relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-32 h-32 bg-emerald-500/10 rounded-full blur-3xl"></div>
                    <div className="flex items-center justify-between mb-4 relative z-10">
                        <div className="p-2.5 bg-emerald-500/20 border border-emerald-500/30 rounded-xl text-emerald-400"><Server size={20} /></div>
                        <div className="flex items-center gap-2">
                            <span className="text-[9px] font-black text-emerald-500/50 uppercase tracking-widest bg-black/20 px-3 py-1 rounded-full border border-emerald-500/20">Uptime</span>
                        </div>
                    </div>
                    <div className="relative z-10">
                        <p className="text-xl font-black text-white tracking-tighter uppercase flex items-center gap-3">
                            <ShieldCheck className={health?.system.dbStatus === 'Conectado' ? "text-emerald-400" : "text-rose-400"} size={24} /> 
                            {health?.system.dbStatus === 'Conectado' ? 'Salud OK' : 'Alerta'}
                        </p>
                        <p className="text-[9px] font-bold text-slate-400 mt-1 uppercase tracking-tight">
                            {health?.system.uptime || 0}h Online
                        </p>
                    </div>
                </div>
